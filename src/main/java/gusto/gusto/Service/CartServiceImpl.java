...
import gusto.gusto.Repo.CartItemRepository;
import gusto.gusto.Repo.CartRepository;
...
public class CartServiceImpl implements CartService{
    ...
    public List<CartDTO> getAllCarts() {
        ...
        if (carts.size() == 0)
            throw new APIException("no resource found");



        List<CartDTO> cartDTOS = carts.stream().map(cart -> {
            CartDTO cartDTO = modelMapper.map(cart, CartDTO.class);
            List<ProductDTO> products = (List<ProductDTO>) cart.getCartItems().stream().map(p -> modelMapper.map(p.getProduct(), ProductDTO.class));
